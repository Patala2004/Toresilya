...
public class MapPainter
{
    ...
    {
        if (roomScript.north)
        { // If it has a corridor on the north -> Dont draw walls where the hallway will pass trough (leave an empty space)
    ...
    private String getDirection(int x, int y)
    {
        Debug.Log(x + " - " + y);
        if (x == -1)
        { // Current node is left of parent node -> draw to the right
            return "East";
        }
        else if (x == 1)
        {
        ...
        { // Current node is bellow parent node -> drow north
            return "North";
        }
        else if (y == 1)
        {
